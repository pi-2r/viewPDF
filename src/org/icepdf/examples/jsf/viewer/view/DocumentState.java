...
public class DocumentState implements Serializable {
	...
	private static final Logger logger = Logger.getLogger(DocumentState.class
			.toString());
	...
	private ArrayList<TreeNode> outline;
	...
	public DocumentState(String documentPath, boolean sharedSession) {
		documentName = documentPath.substring(documentPath
	...
	public DocumentState(DocumentState documentState) {
		...
		this.documentName = documentState.getDocumentName();
		this.documentPath = documentState.getDocumentPath();
	...
	/**
	 * Open the PDF document wrapped by this object. If their is already a
	 * document assigned to this document it is closed before the current
	 * documentPath is loaded.
	 */
	public void openDocument() throws PDFException, IOException,
			PDFSecurityException {
		System.out.println("dans openDocument");
		// get reference to application scoped document cache.
		DocumentCache documentCache = (DocumentCache) FacesUtils
				.getManagedBean("documentCache");
		...
		synchronized (documentLock) {
			...
			if (document == null) {
				...
				if (sharedSession) {
					System.out.println("documentCache.get(documentPath) ==> "
							+ documentCache);

					Iterator it = documentCache.entrySet().iterator();
					while (it.hasNext()) {
					...
					Document documentReference = documentCache
							.get(documentPath);
			...
			if (outlines != null && outlines.getRootOutlineItem() != null) {
				...
				DefaultMutableTreeNode rootItem = new OutlineItemTreeNode(
						document.getPageTree(), outlines.getRootOutlineItem());

				outline = new ArrayList<TreeNode>();
				outline.add(rootItem);
	 ...
	 * 
	 * @return number of pages in document, -1 if the number of pages could not
	 *         be determined.
	 */
	...
	protected Image getPageImage() {
		synchronized (documentLock) {
			if (document != null) {
				...
				if (pageCursor < 1) {
					pageCursor = 1;
				} else if (pageCursor > document.getPageTree()
						.getNumberOfPages()) {
					pageCursor = document.getPageTree().getNumberOfPages();
				}

				return document.getPageImage(pageCursor - 1,
	...
	public void calculatePageImageSize() {
		synchronized (documentLock) {
			if (document != null && document.getCatalog() != null) {
				pageSize = document.getPageDimension(pageCursor - 1, rotation,
						zoom);
			} else {
	...
	public ArrayList<TreeNode> getOutline() {
		return outline;
	}
	...
	public void setOutlineCollapsed(boolean outlineCollapsed) {
		this.outlineCollapsed = outlineCollapsed;
	}
	 ...
	 */
	public class PDFResource extends javax.faces.application.Resource implements
			Serializable {
